...
int main() {
	...
	printf("Testing our adventurerCard()\n");
	...
	for (i = 0; i <= 18; i++) {
		...
		if (origDeck[i] != deckCheck[i]) {
			//Print that test failed
			printf("FAILED: The supply deck was changed by adventurerCard\n");
			failed = 1;
		}
	}
	//If no test failed
	if (failed != 1) {
		printf("adventurerCard works as it should\n");
	}
