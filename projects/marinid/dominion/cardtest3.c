...
int main() {
	...
	//Test to see if discard pile is larger than when great hall was played
	int anotherAction = game.numActions;
	if (anotherAction != (curAction + 1)) {
		//Print that test failed
		printf("FAILED: Player that played great hall did not discard any cards\n");
		failed = 1;
	}
	...
	//If no test failed
	if (failed != 1) {
		printf("great_hallFunct works as it should\n");
	}
